...
This workspace is being turned into a backend service for products.
...
- Elasticsearch, RabbitMQ, and Redis are not wired in yet.
- The current code gives you the product domain first, which is the correct foundation before adding those services.

### First steps to run the project

1. Create a PostgreSQL database, for example `marketplace`, and a test database `marketplace_test`.
2. Update `config/db.php` and `config/test_db.php` with your local PostgreSQL username and password.
3. Run the migration:
...
4. Open the app in the browser and use the product endpoints from the controller.
